...
function ViewCart({ cartItems, setCartItems, products, onRemoveFromCart }) {
  useEffect(() => {
    getCartList();
  ...
  function displayProductList() {
    if (cartItems.length === 0) {
      return (
        <>
          <p className="emptylbl">Your Cart is Empty!</p>
        ...
        <div>
          ...
          <div className="checkOutcon">
            <li className="list-group-item d-flex justify-content-between align-items-center">
              ...
              <Link to={'/shipping-details'} > <button className="btn btn-light">Proceed to Checkout</button> </Link>
            </li>
          </div>
        </div>
      );
